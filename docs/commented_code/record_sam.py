class RecordSam:
    ...
    def __repr__(self):
        ...
        -------
        str
            Строка в формате: <RecordSam id={идентификатор}>
        """
        return f"<RecordSam id={self.id}>"


class SequenceRecordSam(RecordSam):
    """
    Класс для хранения последовательностей в форматах FASTA/FASTQ.
    
    Наследует от Record и добавляет информацию о биологической 
    ...
    --------
    FASTA запись:
    >>> seq_rec = SequenceRecordSam("seq1", "ATCG")
    >>> print(seq_rec.sequence)
    ATCG
    
    FASTQ запись:
    >>> qual = [40, 40, 39, 38]
    >>> fastq_rec = SequenceRecordSam("read1", "ATCG", qual)
...
class AlignmentRecordSam(RecordSam):
    """
    ...
    Examples
    --------
    >>> aln = AlignmentRecordSam("read1", "chr1", 1000, "50M", 60)
    >>> print(aln)
    <AlignmentRecordSam id=read1, chr1:1000-1050, MAPQ=60, FLAG=0>
    """
    ...
    def __repr__(self):
        ...
        -------
        str
            Строка в формате: <AlignmentRecordSam id={id}, {chrom}:{start}-{end}, 
            MAPQ={mapq}, FLAG={flag}>
        """
        return (
            f"<AlignmentRecordSam id={self.id}, {self.chrom}:{self.start}-{self.end}, "
            f"MAPQ={self.mapq}, FLAG={self.flag}>"
        )


class VariantRecordSam(RecordSam):
    """
    ...
    --------
    >>> info = {"DP": 50, "AF": 0.25}
    >>> var = VariantRecord("chr1", 123456, "A", "T", info)
    >>> print(var)
    <VariantRecordSam chr1:123456 A>T>
    """
    ...
    def __repr__(self):
        ...
        -------
        str
            Строка в формате: <VariantRecordSam {chrom}:{pos} {ref}>{alt}>
        """
        return f"<VariantRecordSam {self.chrom}:{self.pos} {self.ref}>{self.alt}>"